...
@njit
def _applyOverlay(background: cv2.Mat, overlay: cv2.Mat, overlayAlpha, startX, startY, endX, endY, overlayStartX, overlayStartY, overlayEndX, overlayEndY) -> cv2.Mat:
    """
...
def drawCursor(frame: cv2.Mat, position: tuple[int, int], colorRGB: tuple[int, int, int], radius: int, thickness: int) -> None:
    """
...
def overlayImagePNG(background: cv2.Mat, overlay: cv2.Mat, x: int, y: int, width: int, height: int) -> cv2.Mat:
    ...
    try:
        ...
        if overlayStartX < overlayEndX and overlayStartY < overlayEndY:
            ...
            background = _applyOverlay(background, overlay, overlayAlpha, startX, startY,
